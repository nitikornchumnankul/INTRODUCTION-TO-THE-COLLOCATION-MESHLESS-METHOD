%---------------------------MQ_2ndDerivative_Approx_SinXCosY xxxx------------------------------------
clear all
clc
esp=input('Parameter :')
...
for i=1:LL
    ...
    y1(i,j)=(yyy(i)-ykn(j));
    Bmq1(i,j)=((esp^2)+(y1(i,j))^2).*inv(((r1(i,j)^2)+esp^2)^(1.5));
    end
end
...
for i=1:Lext
    for j=1:Lext
         Zext(i,j)=(-sin(Xext(i)).*(cos(Yext(j))));%*********
